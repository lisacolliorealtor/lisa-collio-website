...
- [x] **Managing Broker, RE/MAX Results / The Viruez Team** — advertising and legal content approved. **Verbal approval, in office, 18 August 2026.** Scope of what was actually reviewed and approved, recorded here so this box doesn't read as broader than the conversation was:
  ...
  - **Both "NOT approved" items above were reviewed and approved on 19 August 2026** — see the 19 August box below. **This 18 August entry is left exactly as written**, because it records what was true on 18 August and what the broker had actually seen at that point. The approval was genuinely partial by scope for one day; overwriting it would erase that, and the two-stage history is the accurate record.
- [x] **Managing Broker** — Fair Housing review of ad targeting and audience practices (§4.8). **Approved. Verbal, in office, 19 August 2026.** Opened 18 August as explicitly out of scope; closed the following day in the same visit that covered the banner and the change notice. What was reviewed and approved, stated precisely rather than as a bare tick: the housing **special-ad-category** restrictions as set out in §4.8 — declaration required on every campaign, targeting by age, sex, ZIP code and income removed, audience exclusion disabled, Meta's 15-mile radius minimum (wider than Goshen-to-Elkhart), lookalike audiences unavailable and Special Ad Audiences discontinued October 2023 with no replacement; the practices that remain permitted and are what Lisa asked for — custom audiences from her own data, Site visitors captured by her own pixel, uploaded client lists, retargeting prior visitors with listings; and **the two obligations §4.8 identifies as Lisa's rather than the platform's** — that declaring the housing category is hers on every campaign, and that how she constructs an audience is her Fair Housing exposure even where the platform permits it (an uploaded list skewing along a protected characteristic is a steering problem regardless of what the upload allowed).
- [x] **Managing Broker, RE/MAX Results / The Viruez Team — second pass. Verbal approval, in office, 19 August 2026.** Closes both items the 18 August pass left open, and covers one thing built since. Scope, stated precisely:
  - **The cookie consent banner, Google tag, and Meta pixel** as built in PR #143 and #144 and live on `lisacolliorealtor.netlify.app` — the walkthrough of the live banner that the 18 August entry required before DNS cutover. Reviewed as built and running, not as a specification.
  - **Fair Housing review of ad targeting and audience practice (§4.8)** — full scope in the box above.
  - **The Privacy Policy change notice as built in PR #146, both languages** — `/privacy/` and `/es/privacidad/`. New user-facing legal copy; the 18 August approval covered the site only as it stood after PR #141 and could not have reached it.
  - **NOT approved, because approving the targeting practice is not approving the ads:** ad creative and campaign setup. See the open box below.
- [ ] **Managing Broker** — **ad creative and campaign setup, before any paid campaign runs. Still open, and deliberately so.** The 19 August approval covers *how audiences may be targeted and constructed*; it does not approve a single advertisement. This is the same principle §4.5 already states for automated messages — *"Approving this document does not approve the messages. Templates need their own broker review"* — and §4.8's own recommendation puts campaign setup and audience construction in the same broker pass as creative. The Fair Housing rules locked for Site copy apply verbatim to ad creative: no "walkable," no "quiet," no "desirable," no coded qualitative terms. **Nothing in the 19 August approval lets a campaign go live without this pass.**
- [ ] **Brokerage counsel** — automated-voice consent flow reviewed against IC 24-5-14 and the TCPA (§4.7). *Required before voice features are enabled, not before this document is approved.* **Still open regardless of broker approval** — the broker's sign-off does not close this; it requires counsel specifically. **Reaffirmed 19 August 2026:** the second broker pass did not touch it and could not have. This box gates **automated voice features only — it does not gate launch**, and voice is not switched on.
- [x] Effective dates set on both language versions, matched — **set to 19 August 2026 / 19 de agosto de 2026, corrected on 19 August 2026.** This box was ticked at sign-off on 18 August when the mechanism was satisfied but the substance was not: both pages still carried *Effective date: July 25, 2026* / *Fecha de vigencia: 25 de julio de 2026* — matched to each other, but a month earlier than the revisions above, so the page disclosed advertising cookies, retargeting and automated voice messaging under a date predating all of it. Corrected in the Privacy effective-dates PR. Terms and Términos de Uso deliberately left at 25 July (text unrevised); Accessibility and Fair Housing likewise.
- [x] Build authorized — PR A (#141), merged. ~~PR B not yet built/authorized under this scope note.~~ **Updated 19 August 2026:** PR B is built, merged and live — the cookie consent banner, Google tag and Meta pixel shipped in #143 with presentation fixes in #144, and the broker reviewed them running on `lisacolliorealtor.netlify.app` in the 19 August pass above. The struck clause recorded the state on 18 August and is left visible rather than deleted; it would otherwise contradict the box three lines above it.
